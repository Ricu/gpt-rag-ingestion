...
def prepare_context_and_render(template_name: str, template_dir: str, label_filter: str = LABEL_FILTER) -> Tuple[Optional[dict], dict]:
    ...
    def extract_embedding_model_info() -> dict:
        """Extract embedding model information from MODEL_DEPLOYMENTS JSON"""

        embedding_info = {
            "deployment_name": os.environ.get("AZURE_EMBEDDING_MODEL_DEPLOYMENT"),
            "model_name": os.environ.get("AZURE_EMBEDDING_MODEL_DEPLOYMENT"), # same as deployment for now
    ...
    def extract_gpt_model_info() -> dict:
        """Extract GPT model information from MODEL_DEPLOYMENTS JSON for knowledge agents"""

        gpt_info = {
            "deployment_name": os.environ.get("AZURE_GENERATION_MODEL_DEPLOYMENT"),
            "model_name": os.environ.get("AZURE_GENERATION_MODEL_DEPLOYMENT"), # same as deployment for now
            "endpoint": os.environ.get("AZURE_OPENAI_ENDPOINT"),
            "api_version": os.environ.get("AZURE_OPENAI_API_VERSION")
        }
    ...
    embedding_model_info = extract_embedding_model_info()
    ...
    gpt_model_info = extract_gpt_model_info()
    if gpt_model_info:
...
def execute_setup(defs: Optional[dict], context: dict):
    if defs is None:
        ...
        return
    cred = os.environ.get("AZURE_SEARCH_KEY")
    ...
    if not search_endpoint:
        logging.error("❗️ AZURE_SEARCH_ENDPOINT not found in App Configuration; skipping Azure Search setup.")
        return
    if not api_version:
        logging.error("❗️ AZURE_SEARCH_API_VERSION not found in .env; skipping Azure Search setup.")
        return
...
if __name__ == "__main__":
    ...
    template_dir = current_file_path
